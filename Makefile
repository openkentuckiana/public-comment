...
test: 
	@docker-compose run -e DJANGO_SETTINGS_MODULE=public_comment.settings.test --rm app ./wait-for-it.sh db:5432 --timeout=60 -- python ./manage.py test --keepdb $(labels)

testwithcoverage:
	@docker-compose run -e DJANGO_SETTINGS_MODULE=public_comment.settings.test --rm app bash -c "./wait-for-it.sh db:5432 --timeout=60 -- coverage run --source='.' ./manage.py test --keepdb --noinput && coverage html"

testwithcoverage-codecov: build
	@docker-compose run -e DJANGO_SETTINGS_MODULE=public_comment.settings.test -e GIT_SHA=$(GIT_SHA) -e CODECOV_TOKEN=$(CODECOV_TOKEN) --rm app bash -c "./wait-for-it.sh db:5432 --timeout=60 -- coverage run --source='.' ./manage.py test --keepdb --noinput && codecov --commit=$(GIT_SHA)"
...
attach:
	docker attach public-comment_app_1

attach-celery:
	docker attach public-comment_celery_1

bash-running:
	docker exec -it public-comment_app_1 bash

format:
	@docker-compose run --rm app black . -l 128
	@docker-compose run --rm app isort --atomic .
